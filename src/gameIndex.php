...
class Game {
    ...
    function game_play($link) {
        ...
        for ($pos = 0; $pos < pow($this->grid_size, 2); $pos++) {
            ...
            if ($link) {
                ...
                echo '<td style="text-align:center;' . (in_array($pos, $this->winning_line[0]) ? ' background-color: #90EE90;' : ' opacity: 0.5;' ) . '"><div style="padding: 1em;">' . $this->position[$pos] . ($this->debug ? ('<br /><span style="font-size:60%;">' . $pos . ':(' . $row . ',' . (($pos % $this->grid_size) + 1) . ')</span>') : '') . '</div></td>';
    ...
    function game_message($message) {
        ...
        switch ($message) {
            ...
            case 'o-win':
                ...
                echo '<strong>The computer is the winner of this game.  Care for a rematch?  You\'ll probably lose again haha...</strong>';
        ...
        if ($newGame) {
            ...
            if ($this->debug) {
                ...
                echo '<br /><br /><a draggable="false" href="' . $_SERVER['PHP_SELF'] . '?size=' . $this->grid_size . '&debug" style="-webkit-appearance: button; -moz-appearance: button; appearance: button; text-decoration: none; color: initial; padding: 0.5em;">Click here to start a new game!</a>';
    ...
    function game_stats() {
        ...
        if ($games > 0) {
            ...
            echo "Total Computer Wins / Player Defeats: <strong>" . $loses . "</strong> ( " . $loseP . "% )<br />";
